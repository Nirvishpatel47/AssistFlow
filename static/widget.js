(function () {
  ...
  if (window.__CHAT_WIDGET_INITIALIZED__) {
  ...
  // Check if widget DOM elements already exist
  if (document.getElementById("chat-bubble") && document.getElementById("chat-box")) {
    console.log("Chat widget already exists in DOM");
    return;
  }
  ...
  let isTyping = false;
  let animationTimeoutIds = []; // Track all animation timeouts
  ...
  // Extremely lightweight, fast regex parser to convert basic Markdown to clean HTML
  function parseMarkdown(text) {
    if (!text) return "";
    
    let html = text;

    // 1. Escape basic HTML characters to prevent XSS injection
    html = html
      .replace(/&/g, "&amp;")
      .replace(/</g, "&lt;")
      .replace(/>/g, "&gt;");
    ...
    // 2. Headers (### Heading)
    html = html.replace(/^### (.*$)/gim, '<h3 style="margin: 12px 0 6px 0; font-size: 14px; font-weight: 600; color: inherit;">$1</h3>');
    html = html.replace(/^## (.*$)/gim, '<h2 style="margin: 14px 0 8px 0; font-size: 15px; font-weight: 600; color: inherit;">$1</h2>');
    html = html.replace(/^# (.*$)/gim, '<h1 style="margin: 16px 0 10px 0; font-size: 16px; font-weight: 700; color: inherit;">$1</h1>');

    // 3. Bold Bold (**text**)
    html = html.replace(/\*\*(.*?)\*\*/g, '<strong style="font-weight: 600;">$1</strong>');

    // 4. Unordered Lists (* item or - item)
    // First, convert bullet items to <li> tags
    html = html.replace(/^\s*[\*\-]\s+(.*$)/gim, '<li style="margin-left: 16px; margin-bottom: 4px; list-style-type: disc;">$1</li>');
  ...
  function addMessage(text, type, animate = false) {
    ...
    // Parse Markdown elements before displaying or animating them
    ...
    if (animate && type === "bot") {
      ...
      // Get all child nodes (text nodes and elements)
      ...
      const typeNextNode = () => {
        // Check if widget still exists and animation should continue
        ...
        if (nodeIdx < nodes.length) {
          ...
          if (currentNode.nodeType === Node.TEXT_NODE) {
            if (charIdx < currentNode.textContent.length) {
              bubble.innerHTML += currentNode.textContent.charAt(charIdx);
              charIdx++;
              if (!isUserScrolling && messages) {
                messages.scrollTop = messages.scrollHeight;
              }
              const timeoutId = setTimeout(typeNextNode, Math.floor(Math.random() * 8) + 2);
              animationTimeoutIds.push(timeoutId);
            } else {
              charIdx = 0;
              nodeIdx++;
              const timeoutId = setTimeout(typeNextNode, 40);
              animationTimeoutIds.push(timeoutId);
            }
          } else {
            // Append HTML element clone
            bubble.appendChild(currentNode.cloneNode(true));
            nodeIdx++;
            if (!isUserScrolling && messages) {
              messages.scrollTop = messages.scrollHeight;
            }
      ...
      // Clear any existing animation timeouts before starting new one
      animationTimeoutIds.forEach(id => clearTimeout(id));
      animationTimeoutIds = [];
      
      typeNextNode();
    } else {
      // If no animation is requested (e.g. user messages), inject parsed HTML instantly
